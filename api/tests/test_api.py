...
DUMMY_STATE = [
    DUMMY_CHILLY_RESP, DUMMY_NETWORK_ORDER, DUMMY_WAN_STATE_RESP,
    DUMMY_SIGNAL_RESP
]
BATTERY_SIDE_EFFECTS = [
    'CHARGING', '98', u'{"charging current": 0, "voltage": 12}'
]
CONNECTED_CLIENTS_SIDE_EFFECT = (
...
PSUTIL_TEMP_SIDE_EFFECT = {
    'coretemp': [
        shwtemp(label='Core 0', current=55.0, high=110.0, critical=110.0),
...
EXPECTED_ETHERNET1_DISCONNECTED = dict(
    ...
    available=False,
    connected=False,
    info=dict(dhcp_enabled=True, network=dict(gateway='', dns='')))
...
EXPECTED_NETWORK_RESP = dict(
    ...
    connected_clients=1,
    connection=dict(
        connection_type='3G', up_speed=0, down_speed=0, signal_strength=100))
...
"""
EXPECTED_BATTERY = dict(
    state='CHARGING', battery_level=98, charging_current=0, voltage=12)

# wifi
EXPECTED_WIFI_UNCOFIGURED = dict(
    id='WIFI1', name='Wireless 1', connected=False, available=False, info={})
...
@pytest.mark.skipif(
    sys.platform == 'darwin', reason="spwd not available on MAC")
def test_get_auth_token_as_system_user(client, headers):
    _auth = dict(login=ROOT_USER, password=ROOT_PASSWORD)
    with mock.patch(
            'local_api.apiv1.models.check_system_login', side_effect=[True]):
        resp = client.post(
            '/api/v1/auth',
            content_type='application/json',
            data=json.dumps(_auth))
...
def test_system_api(client, headers):
    with mock.patch(
            'local_api.apiv1.utils.get_interface_speed', side_effect=[(0, 0)]):
...
def test_system_battery_api(client, headers):
    with mock.patch(
            'local_api.apiv1.utils.get_interface_speed', side_effect=[(0, 0)]):
        with mock.patch(
                'local_api.apiv1.utils.uci_get', side_effects=['ALWAYS_ON']):
            with mock.patch(
                    'local_api.apiv1.utils.run_command',
                    side_effect=DUMMY_STATE):
                with mock.patch(
                        'local_api.apiv1.utils.get_battery_status',
                        side_effect=[EXPECTED_BATTERY]):
                    resp = client.get('/api/v1/system', headers=headers)
                    assert (resp.status_code == 200)
                    payload = load_json(resp)
                    assert 'battery' in payload
    ...
    with mock.patch(
        ...
        with mock.patch(
            ...
            with mock.patch(
                    'local_api.apiv1.utils.run_command',
                    side_effect=[
                        DUMMY_CHILLY_RESP, DUMMY_NETWORK_ORDER,
                        DUMMY_WAN_STATE_RESP, '31', BAT_SIDE_EFFECT
                    ]):
                with mock.patch(
                        'local_api.apiv1.utils.get_battery_status',
                        side_effect=[EXPECTED_BATTERY]):
...
def test_get_power_config_not_configured(client, headers):
    not_configured = dict(
        configured=False, mode=None, battery=EXPECTED_BATTERY)
    with mock.patch(
            'local_api.apiv1.utils.run_command',
            side_effect=[BAT_SIDE_EFFECT]):
        with mock.patch(
                'local_api.apiv1.soc.get_power_config',
...
def test_get_power_config_configured(client, headers):
    configured = dict(
        configured=True, mode='ALWAYS_ON', battery=EXPECTED_BATTERY)
    with mock.patch('local_api.apiv1.soc.uci_get', side_effect=['ALWAYS_ON']):
        with mock.patch(
                'local_api.apiv1.utils.run_command',
                side_effect=[BAT_SIDE_EFFECT]):
            resp = client.get(
...
def test_get_sim_networks(client, headers):
    resp = client.get('/api/v1/networks/sim/', headers=headers)
    assert resp.status_code == 200
    payload = load_json(resp)
...
def test_get_sim_network(client, headers):
    resp = client.get('/api/v1/networks/sim/SIM1', headers=headers)
    assert resp.status_code == 200
    payload = load_json(resp)
...
def test_patch_sim(client, headers):
    test_payload = dict(
        configuration=dict(pin="1234", network=dict(apn="internet")))
...
def test_get_ethernet_networks(client, headers):
    with mock.patch(
        ...
        with mock.patch(
                'local_api.apiv1.ethernet.psutil.net_if_addrs',
                side_effect=[{}]):
            resp = client.get('/api/v1/networks/ethernet/', headers=headers)
            assert resp.status_code == 200
            payload = load_json(resp)
...
def test_get_ethernet_networks_single(client, headers):
    with mock.patch(
        ...
        with mock.patch(
                ...
                side_effect=[{}]):
            resp = client.get(
                '/api/v1/networks/ethernet/ETHERNET1', headers=headers)
            assert resp.status_code == 200
            payload = load_json(resp)
...
def test_get_ethernet_networks_unknown(client, headers):
    with mock.patch(
            ...
            side_effect=[DISCONNECTED_UCI_STATE]):
        resp = client.get(
            '/api/v1/networks/ethernet/ETHERNET9', headers=headers)
        assert resp.status_code == 404
        assert load_json(resp)


def test_patch_ethernet_dhcp(client, headers):
    test_payload = dict(configuration=dict(dhcp_enabled=True))
    with mock.patch(
            'local_api.apiv1.utils.run_command',
            side_effect=[DISCONNECTED_UCI_STATE]):
        with mock.patch(
                ...
                side_effect=[{}]):
            resp = client.patch(
                '/api/v1/networks/ethernet/ETHERNET1',
                content_type='application/json',
                data=json.dumps(test_payload),
                headers=headers)
            assert resp.status_code == 200
...
def test_patch_ethernet_static_invalids(client, headers):
    test_payload = dict(
        configuration=dict(
            dhcp_enabled=False,
            network=dict(ipaddr='x', netmask='y', dns='8.8.8.8 localhost')))
    with mock.patch(
            'local_api.apiv1.utils.run_command',
            side_effect=[DISCONNECTED_UCI_STATE]):
        resp = client.patch(
...
def test_get_wifi_networks_no_conf(client, headers):
    resp = client.get('/api/v1/networks/wifi/', headers=headers)
    assert resp.status_code == 200
    payload = load_json(resp)
...
def test_patch_wifi_ap_mode(client, headers):
    ...
    with mock.patch(
        ...
        with mock.patch(
                'local_api.apiv1.wifi.uci_get',
                side_effect=['pci0000:00/0000:00:1c.3/0000:04:00.0', '1',
                             '1']):
            resp = client.patch(
                '/api/v1/networks/wifi/WIFI1',
                content_type='application/json',
...
def test_get_wifi_networks_available(client, headers):
    with mock.patch(
            'local_api.apiv1.utils.run_command',
            side_effect=[WIFI_NET_STATE, WIFI_UCI_STATE]):
        with mock.patch('local_api.apiv1.wifi.uci_get', side_effect=['1']):
            resp = client.get('/api/v1/networks/wifi/', headers=headers)
            assert resp.status_code == 200
            payload = load_json(resp)
...
def test_get_software(client, headers):
    resp = client.get('/api/v1/system/software', headers=headers)
    assert resp.status_code == 200
    payload = load_json(resp)
...
def test_get_diagnostics(client, headers):
    ...
    with mock.patch(
        ...
        with mock.patch(
            ...
            with mock.patch(
                    'local_api.apiv1.utils.psutil.sensors_temperatures',
                    side_effect=[PSUTIL_TEMP_SIDE_EFFECT]):
                resp = client.get(
                    '/api/v1/system/diagnostics', headers=headers)
                assert resp.status_code == 200
                payload = load_json(resp)
                ...
                assert payload['battery'][
                    'temperature'] == EXPECTED_BAT_TEMPERATURE
                assert payload['cpu'][
                    'temperature'] == EXPECTED_CPU_TEMPERATURE
